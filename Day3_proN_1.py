...
def lantern_power(n, A):
    # Step 1: Build suffix maxima (right to left)
    suffix_max = [0] * n
    suffix_max[-1] = A[-1]
    for i in range(n - 2, -1, -1):
        suffix_max[i] = max(A[i], suffix_max[i + 1])

    # Step 2: Traverse left to right, maintain prefix min, compute answer
    total = 0
    prefix_min = A[0]
    for i in range(n):
        prefix_min = min(prefix_min, A[i])
        total += abs(suffix_max[i] - prefix_min)

    return total
n = 3
arr = [1,3,2]
print(lantern_power(n,arr))
